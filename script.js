...
/* The canvas paints a living iridescent field (drifting blue / violet / gold     */
/* light centers), with the JCC -> "Juliani Consulting Company" wordmark rendered */
/* as crisp white ASCII on top. Pointer movement stirs the field.                 */
  ...
  const render = (now) => {
    ...
    if (machineState) {
      ...
      machineState.textContent = reducedMotion
        ? "IDENTITY LOCKED"
        : onEdge ? "MORPH" : (progress >= 0.05 && progress <= 0.95 ? `${mode.toUpperCase()} / LIVE` : `${mode.toUpperCase()} / LIVE`);
    }
  };
